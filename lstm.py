    ...
    def __init__(self, state_size, num_classes,
        ...
        def __graph__():
            ...
            init_state = tf.placeholder(
                shape=[2, None, state_size], dtype=tf.float32, name='initial_state')
            # initializer
    ...
    def generate(self, idx2w, w2idx, num_words=100, separator=' '):
        ...
        with tf.Session() as sess:
            ...
            for i in range(num_words):
                ...
                #
                # forward propagation
                preds, state_ = sess.run(
                    [self.predictions, self.last_state], feed_dict=feed_dict)
                #
                # set flag to true
                state = True
                #
                ...
                current_word = np.random.choice(
                    preds.shape[-1], 1, p=np.squeeze(preds))[0]
